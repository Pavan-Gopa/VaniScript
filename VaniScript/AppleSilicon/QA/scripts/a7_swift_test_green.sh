#!/usr/bin/env bash
# A7 gate: swift test green, soft-warn if < 331 tests (FEEDBACK/ADR claim 331/47).
# ENV-ONLY sandbox disposition matches a2/a4/a5/a6_swift_test_green.
set -uo pipefail
AS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$AS_DIR"

echo "A7 swift test green — cwd: $AS_DIR"
out_file="$(mktemp -t a7_swift_test.XXXXXX)"
...
if [[ "$swift_rc" -ne 0 && "$has_summary" -eq 0 ]] && \
  ...
  echo "RESULT: ENV-ONLY (a7_swift_test_green) — treat as skip, not FAIL."
  exit 0
fi
...
[[ "$tests_count" -lt 331 ]] && echo "WARN: executed $tests_count tests (< 331 claimed) — count drift, not failing."

echo "OK: swift test green ($tests_count tests, 0 failures)"
echo "RESULT: PASS (a7_swift_test_green)"
exit 0
