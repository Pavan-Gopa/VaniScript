#!/usr/bin/env bash
# A6 gate: swift test green, soft-warn if < 320 tests (FEEDBACK claim 320/46).
# ENV-ONLY sandbox disposition matches a2/a4/a5_swift_test_green.
set -uo pipefail
AS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$AS_DIR"

echo "A6 swift test green — cwd: $AS_DIR"
out_file="$(mktemp -t a6_swift_test.XXXXXX)"
...
if [[ "$swift_rc" -ne 0 && "$has_summary" -eq 0 ]] && \
  ...
  echo "RESULT: ENV-ONLY (a6_swift_test_green) — treat as skip, not FAIL."
  exit 0
fi
...
echo "RESULT: PASS (a6_swift_test_green)"
exit 0
