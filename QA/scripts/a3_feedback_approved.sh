...
if [[ -n "$current_step" && "$current_step" != "A3" ]]; then
  echo "NOTE: current_step='$current_step' (not A3); A3 FEEDBACK gate is N/A."
  echo "RESULT: PASS (a3_feedback_approved, step-aware N/A)"
  exit 0
fi
