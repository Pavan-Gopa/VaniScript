#!/usr/bin/env bash
# A3: STATE.yaml current_step is A3; implementation+review approved.
# Step-aware: when the track advances past A3 this gate is N/A (PASS).
set -uo pipefail
...
if [[ "$current_step" != "A3" ]]; then
  echo "NOTE: current_step='$current_step' (not A3); A3 STATE gate is N/A."
  echo "RESULT: PASS (a3_state_yaml_a3, step-aware N/A)"
  exit 0
fi
